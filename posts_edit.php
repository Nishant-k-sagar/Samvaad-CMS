...
if (isset($_POST['title'])) {
    ...
    if ($stm = $connect->prepare('UPDATE posts SET title = ?, content = ?, date = ? WHERE id = ?')) {
        ...
        set_message("Post ID " . htmlspecialchars($_GET['id']) . " has been updated.");
        header('Location: posts.php');
        die();
    } else {
        $message = '<div class="alert alert-danger mb-4">Could not prepare post update statement!</div>';
    }
}
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow border-0 rounded">
                <div class="card-body">
                    <h2 class="card-title mb-4 text-center">Edit Post</h2>
                    <?php echo $message; ?>
...
<?php
    ...
    echo '<div class="container mt-5"><div class="alert alert-danger">No post selected.</div></div>';
    die();
}
